...
use crate::neunet::utils::optimization::*;
        ...
        fn update_weights(iteration: u32, hp: &HyperParams, nn: &mut NNModel) {
            match hp.optimization_type {
                OptimizationType::Momentum =>
                    momentum(iteration, hp.momentum_beta, hp.learning_rate, nn),
                OptimizationType::RMSProp =>
                    rms_prop(iteration, hp.rms_prop_beta, hp.learning_rate, nn),
                OptimizationType::Adam =>
                    adam(iteration, hp.momentum_beta, hp.rms_prop_beta, hp.learning_rate, nn),
                _ =>
